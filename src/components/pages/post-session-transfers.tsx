...
function TransferOptionsSection() {
  const [enabled, setEnabled] = useTrackedState(true);
  const [allowAgent, setAllowAgent] = useTrackedState(true);
  const [allowQueue, setAllowQueue] = useTrackedState(true);
  const [allowExternal, setAllowExternal] = useTrackedState(false);

  return (
    <SettingsSection title="Transfer Options" collapsible={false}>
      <SettingsField
        label="Enable post-session transfers"
        description="Allow agents to initiate a transfer after a session ends."
      >
        <SettingsToggle enabled={enabled} onChange={setEnabled} />
      </SettingsField>
      <SettingsField label="Allow transfer to agent">
        <SettingsToggle enabled={allowAgent} onChange={setAllowAgent} />
      </SettingsField>
      <SettingsField label="Allow transfer to queue">
        <SettingsToggle enabled={allowQueue} onChange={setAllowQueue} />
      </SettingsField>
      <SettingsField label="Allow transfer to external number" noBorder>
        <SettingsToggle enabled={allowExternal} onChange={setAllowExternal} />
      </SettingsField>
    </SettingsSection>
  );
}

function TransferBehaviorSection() {
  const [timeout, setTimeout] = useTrackedState("30");
  const [defaultDest, setDefaultDest] = useTrackedState("Last assigned agent");

  return (
    <SettingsSection title="Transfer Behavior" collapsible={false}>
      <SettingsField
        label="Transfer timeout"
        description="Time before a transfer attempt is abandoned."
      >
        <NumberInput value={timeout} onChange={setTimeout} suffix="Seconds" />
      </SettingsField>
      <SettingsField label="Default transfer destination" noBorder>
        <Dropdown
          value={defaultDest}
          options={[
            "Last assigned agent",
...
export function PostSessionTransfersPage({
  onNavigateAttempt,
  sidebarCollapsed,
}: {
