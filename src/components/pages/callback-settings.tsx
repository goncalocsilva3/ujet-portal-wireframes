...
function CallbackOptionsSection() {
  const [enabled, setEnabled] = useTrackedState(true);
  const [offerOnLongWait, setOfferOnLongWait] = useTrackedState(true);
  const [waitThreshold, setWaitThreshold] = useTrackedState("5");
  const [allowScheduled, setAllowScheduled] = useTrackedState(false);

  return (
    <SettingsSection title="Callback Options" collapsible={false}>
      <SettingsField
        label="Enable callbacks"
        description="Allow callers to request a callback instead of waiting on hold."
      >
        <SettingsToggle enabled={enabled} onChange={setEnabled} />
      </SettingsField>
      <SettingsField
        label="Offer callback during long wait times"
        description="Automatically offer a callback when the wait exceeds the threshold."
      >
        <SettingsToggle
          enabled={offerOnLongWait}
          onChange={setOfferOnLongWait}
        />
      </SettingsField>
      <SettingsField label="Wait time threshold">
        <NumberInput
          value={waitThreshold}
      ...
      <SettingsField label="Allow scheduled callbacks" noBorder>
        <SettingsToggle
          enabled={allowScheduled}
          onChange={setAllowScheduled}
        />
      </SettingsField>
    </SettingsSection>
  );
...
function CallbackBehaviorSection() {
  const [maxAttempts, setMaxAttempts] = useTrackedState("3");
  const [timeBetween, setTimeBetween] = useTrackedState("10");
  const [notifyCustomer, setNotifyCustomer] = useTrackedState(true);
  const [priority, setPriority] = useTrackedState("Same as original call");

  return (
    <SettingsSection title="Callback Behavior" collapsible={false}>
      <SettingsField
        label="Maximum callback attempts"
        description="Stop trying after this many failed attempts."
      >
        <NumberInput value={maxAttempts} onChange={setMaxAttempts} />
      </SettingsField>
      <SettingsField label="Time between attempts">
        <NumberInput
          value={timeBetween}
      ...
      <SettingsField label="Callback queue priority">
        <Dropdown
          value={priority}
          options={[
            "Lower than original call",
            "Same as original call",
            "Higher than original call",
          ]}
          onChange={setPriority}
        />
      </SettingsField>
      <SettingsField label="Notify customer of callback time" noBorder>
        <SettingsToggle
...
export function CallbackSettingsPage({
  onNavigateAttempt,
  sidebarCollapsed,
}: {
    ...
    <Provider>
      <div className="flex flex-col gap-4 pb-20">
        <div key={resetKey} className="flex flex-col gap-4">
          <CallbackOptionsSection />
          <CallbackBehaviorSection />
        </div>
